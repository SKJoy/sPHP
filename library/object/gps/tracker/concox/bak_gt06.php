<?php
/*
    Name:           Concox GT-06 GPS tracker
    Purpose:        GPS tracker object
    Author:         Broken Arrow (SKJoy2001@GMail.Com);
    Date modified:  August 06, 2020 10:07 AM
*/
...
//var_dump(["Data" => $Data, "ProtocolNumber" => $ProtocolNumber, "ProtocolData" => $ProtocolData, ]);
	...
	public function EncodeMessageResponse($DecodedData){
		if($DecodedData["ProtocolNumber"] == "01"){
			$Result = $this->ResponseMessage_LogIn($DecodedData, $DecodedData["ProtocolNumber"]);
		}
		elseif($DecodedData["ProtocolNumber"] == "13"){
			$Result = $this->ResponseMessage_Status($DecodedData, $DecodedData["ProtocolNumber"]);
		}
		else{
			$Result = false;
		}
	...
	private function DecodeMessage_Status($Data){
		...
		*/
		...
		$Result = [
			...
			"Voltage" => substr($Data, 2, 2) * 16.66, // Percentile
			"GSMSignal" => substr($Data, 4, 2) * 25, // Percentile
			"Alarm" => $this->Decode_Alarm(substr($Data, 6, 2)),
	...
	private function DecodeMessage_Alarm($Data){
		...
		*/
		...
		$Result = [
			...
			"Latitude" => (hexdec(substr($Data, 14, 8)) / 30000 / 60) * $CourseStatus["LatitudeMultiplier"],
			"Longitude" => (hexdec(substr($Data, 22, 8)) / 30000 / 60) * $CourseStatus["LongitudeMultiplier"],
			...
			"GPSIsValid" => $CourseStatus["GPSIsValid"],
			"GPSMode" => $CourseStatus["GPSMode"],
	...
	private function ResponseMessage_LogIn($DecodedData, $ProtocolNumber){
		$Result = "{$this->Property["PacketStartMarker"]}{$this->Property["CommonResponsePacketLength"]}{$ProtocolNumber}{$DecodedData["Serial"]}{$this->Property["CommonResponseErrorCheck"]}{$this->Property["PacketEndMarker"]}"; // Static D9DC as Error check!!!

		return $Result;
	}

	private function ResponseMessage_Status($DecodedData, $ProtocolNumber){
