...
$ListSearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "MeasureID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
...
$ListColumn = [ // Columns to display
	...
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Capacity") . "", "{$Caption}", FIELD_TYPE_NUMBER),
	new HTML\UI\Datagrid\Column("" . ($Caption = "Measure") . "Name", "{$Caption}", null, ALIGN_CENTER),
...
$ListSearchInterface = "
	...
	" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}" . ($Caption = "Measure") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get(), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", null, true) . "
...
$InputFieldValidation = [
	...
	new HTTP\InputValidation("MeasureID", true, VALIDATION_TYPE_INTEGER),
];

$InputDataVerificationSQL = "
	(
			" . ($Column = "{$Entity}Name") . " = '{$Database->Escape($_POST["{$Column}"])}'
		AND	{$Table["{$Entity}"]->Alias()}." . ($Column = "{$Entity}TypeID") . " = " . intval($_POST["{$Column}"]) . "
	)
...
$InputDefaultValueFromSearchFieldList = "{$Entity}TypeID, MeasureID";
...
$InputInterface = "
	...
	" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Capacity") . "", 100, 1, true, INPUT_TYPE_NUMBER), "{$Caption}", null, true, null) . "
	" . HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Measure") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", null, true, null) . "
...
$ImportField = [
	...
	new Database\Field("" . ($Field = "Measure") . "ID", "{$Field}", null, $Table["{$Field}"], "{$Field}Name"),
