...
if(isset($_POST["btnImport"])){
	...
	if(isset($_POST["{$Entity}DataFile"])){
		$Table["{$Entity}"]->Import("{$Environment->TempPath()}{$_POST["{$Entity}DataFile"]}", [
			...
			new Database\Field("{$Entity}" . ($Field = "Identifier") . "Value", "{$Field}"),
	...
	if(isset($_POST["btnSubmit"])){ // INSERT/UPDATE record
		...
		if($Form->Verify()){ // Form input okay
			if($Table["{$Entity}"]->Get( // Check for duplicate values for UNIQUE columns
				"
					(
						...
						OR	{$Entity}Identifier = '" . str_replace("'", "''", $_POST["{$Entity}Identifier"]) . "'
					)
					AND	{$Entity}ID <> {$EntityID}
				"
			))$Form->ErrorMessage("Name or identifier already exists!");
			...
			if(!$Form->ErrorMessage()){ // Everything looks fine
				...
				], $EntityID ? "{$Entity}ID = {$EntityID}" : null, null, false); // Save information into database
	...
	print "
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				$Application->URL($_POST["_Script"]), // Submission URL
				"
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Name") . "", $Configuration["InputWidth"], null, true), "{$Caption}", false, false, $Configuration["FieldCaptionWidth"]) . "
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Identifier") . "", $Configuration["InputInlineWidth"], null, true), "{$Caption}", false, true, $Configuration["FieldCaptionInlineWidth"]) . "
...
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Identifier") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]}%'" : null;
