...
$EM = new EntityManagement($Table[$Entity = "PaymentMethod"]);
...
$EM->ValidateInput(function($Entity, $Database, $Table, $PrimaryKey, $ID){
	...
	if($Table->Get( // Check for duplicate values for UNIQUE columns
		"
			...
			AND	{$PrimaryKey} <> {$ID}
		"
	, null, null, null, null, null, null))$Result = "Same payment method exists!";
...
if(isset($_POST["btnInput"])){
	...
	$EM->InputUIHTML([
		...
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Identifier") . "", $EM->InputInlineWidth(), null, true), "{$Caption}", null, true, $EM->FieldCaptionInlineWidth()),
