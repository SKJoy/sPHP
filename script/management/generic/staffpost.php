...
$ListSearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "StaffID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
...
$ListSearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "DepartmentID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
$ListSearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "DesignationID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
...
$ListOrderBy = "{$Entity}JoinDate";
$ListOrder = "ASC";
$ListRecordsPerPage = $Configuration["DatagridRowsPerPage"];

$ListColumn = [ // Columns to display
	new HTML\UI\Datagrid\Column("Staff" . ($Caption = "Code") . "", "{$Caption}"),
	...
	new HTML\UI\Datagrid\Column("" . ($Caption = "Department") . "Name", "{$Caption}", null, null),
	new HTML\UI\Datagrid\Column("" . ($Caption = "Designation") . "Name", "{$Caption}", null, null),
...
$ListSearchInterface = "
	" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}" . ($Caption = "Staff") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get(), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", null, null) . "
	...
	" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}" . ($Caption = "Designation") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get(), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", null, true) . "
...
$InputFieldValidation = [
	new HTTP\InputValidation("StaffID", true, VALIDATION_TYPE_INTEGER),
	new HTTP\InputValidation("EnterpriseID", true, VALIDATION_TYPE_INTEGER),
	new HTTP\InputValidation("OrganizationID", null, VALIDATION_TYPE_INTEGER),
	new HTTP\InputValidation("DepartmentID", null, VALIDATION_TYPE_INTEGER),
	new HTTP\InputValidation("DesignationID", true, VALIDATION_TYPE_INTEGER),
];

$InputDataVerificationSQL = "
	(
			{$Table["{$Entity}"]->Alias()}." . ($Column = "StaffID") . " = " . intval($_POST["{$Column}"]) . "
		...
		AND	{$Table["{$Entity}"]->Alias()}." . ($Column = "DepartmentID") . " = " . intval($_POST["{$Column}"]) . "
		AND	{$Table["{$Entity}"]->Alias()}." . ($Column = "DesignationID") . " = " . intval($_POST["{$Column}"]) . "
	)
...
$InputDefaultValueFromSearchFieldList = "StaffID, EnterpriseID, OrganizationID, DepartmentID, DesignationID";

require __DIR__ . "/../../common/entitymanagement_loadexistingdata.php"; // Load existing data

$InputInterface = "
	" . HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Staff") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", null, null, $Configuration["FieldCaptionWidth"]) . "
	" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Join") . "Date", $Configuration["InputDateWidth"], null, null, INPUT_TYPE_DATE), "{$Caption}ing date", true, null, $Configuration["FieldCaptionWidth"]) . "
	...
	" . HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Department") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"]) . "
	" . HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Designation") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"]) . "
...
$ImportField = [
	new Database\Field("" . ($Field = "Staff") . "ID", "{$Field}", null, $Table["{$Field}"], "{$Field}Name"),
	...
	new Database\Field("" . ($Field = "Designation") . "ID", "{$Field}", null, $Table["{$Field}"], "{$Field}Name"),
