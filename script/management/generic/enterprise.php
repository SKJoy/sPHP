...
$ListSearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "CountryID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
...
$ListColumn = [ // Columns to display
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Logo") . "URL", "{$Caption}", FIELD_TYPE_PICTUREURL),
	...
	new HTML\UI\Datagrid\Column("" . ($Caption = "Country") . "Name", "{$Caption}", null, ALIGN_CENTER),
	new HTML\UI\Datagrid\Column("{$Entity}Is" . ($Caption = "Active") . "", "{$Caption}", FIELD_TYPE_BOOLEANICON),
];

$ListAction = [ // Action
	new HTML\UI\Datagrid\Action("{$Environment->IconURL()}organization.png", null, $Application->URL("Management/Generic/Organization")),
...
$ListSearchInterface = "
	...
	" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}" . ($Caption = "Country") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get(), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", null, true) . "
...
$InputDataVerificationSQL = "
	(
		...
		AND	{$Table["{$Entity}"]->Alias()}." . ($Column = "CountryID") . " = " . intval($_POST["{$Column}"]) . "
	)
...
$InputDefaultValueFromSearchFieldList = "CountryID, xSomethingElseID";
...
$InputInterface = "
	...
	" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Short") . "Name", $Configuration["InputWidth"], null, null), "{$Caption} name", true, null, $Configuration["FieldCaptionWidth"]) . "
	" . HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Country") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", null, true, null) . "
	" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Logo") . "", $Configuration["InputFullWidth"], "{$Environment->UploadURL()}{$LowercaseEntity}/" . (isset($_POST["{$Entity}{$Caption}"]) ? $_POST["{$Entity}{$Caption}"] : null) . "", null, INPUT_TYPE_FILE), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"]) . "
