...
$EM = new EntityManagement($Table[$Entity = "Language"]);

$EM->ImportField([
	new Database\Field("{$Entity}" . ($Field = "Name") . "", "{$Field}"),
	new Database\Field("{$Entity}" . ($Field = "NameNative") . "", "{$Field}"),
	new Database\Field("{$Entity}" . ($Field = "Code") . "", "{$Field}"),
	new Database\Field("{$Entity}" . ($Field = "RegionalCode") . "", "{$Field}"),
	...
	if($Table->Get( // Check for duplicate values for UNIQUE columns
		"
			(
				...
				AND	" . ($Column = "{$Entity}Code") . " = '{$Database->Escape($_POST["{$Column}"])}'
				AND	" . ($Column = "{$Entity}RegionalCode") . " = '{$Database->Escape($_POST["{$Column}"])}'
			)
			AND	{$PrimaryKey} <> {$ID}
		"
	, null, null, null, null, null, null))$Result = "Same " . strtolower($Entity->FormalName()) . " with same code with same regional code exists!";
...
$EM->ListColumn([
	...
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Regional") . "Code", "{$Caption}", null, ALIGN_CENTER),
	new HTML\UI\Datagrid\Column("{$Entity}Name" . ($Caption = "Native") . "", "{$Caption}"),
...
if(isset($_POST["btnInput"])){
	...
	$EM->InputUIHTML([
		...
		HTML\UI\Field(HTML\UI\Input("{$Entity}Name" . ($Caption = "Native") . "", $EM->InputInlineWidth(), null, null), "{$Caption}", null, true, $EM->FieldCaptionInlineWidth()),
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Code") . "", $EM->InputWidth(), null, true), "{$Caption}", true, null, $EM->FieldCaptionWidth()),
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Regional") . "Code", $EM->InputInlineWidth(), null, null), "{$Caption} code", null, true, $EM->FieldCaptionInlineWidth()),
...
$EM->SearchSQL([
	...
	SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}NativeName") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null,
	SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Code") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null,
	SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}RegionalCode") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null,
...
$EM->SearchUIHTML([
	...
	HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Native") . "Name", 200), "{$Caption}", null, true),
	HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Code") . "", 200), "{$Caption}", null, true),
	HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Regional") . "Code", 200), "{$Caption}", null, true),
