	...
	print "
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				$Application->URL($_POST["_Script"]), // Submission URL
				"
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Name") . "", $Configuration["InputWidth"], null, true, null, null, null, ["OnChange" => "Generate{$Entity}Identifier();", ], "{$Entity}Name"), "{$Caption}", null, null, $Configuration["FieldCaptionWidth"]) . "
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Identifier") . "", $Configuration["InputInlineWidth"], null, true, null, null, null, ["xOnFocus" => "", ], "{$Entity}Identifier"), "{$Caption}", null, true, $Configuration["FieldCaptionInlineWidth"]) . "
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Weight") . "", 120, 100, true, INPUT_TYPE_NUMBER), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"]) . "
			...
			<script>
				function Generate{$Entity}Identifier(){
					objIdentifier = document.getElementById('{$Entity}Identifier');

					if(!objIdentifier.value){
						objIdentifier.value = document.getElementById('{$Entity}Name').value.replace(' ', '_').toUpperCase();
					}
				}
			</script>
		</div>
