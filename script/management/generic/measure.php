...
$ListSearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Symbol") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null;
...
$ListColumn = [ // Columns to display
	...
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Symbol") . "", "{$Caption}"),
...
$ListSearchInterface = "
	...
	" . HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Symbol") . "", 200), "{$Caption}", null, true) . "
...
$InputFieldValidation = [
	...
	new HTTP\InputValidation("{$Entity}Symbol", true),
...
$InputDefaultValueFromSearchFieldList = "{$Entity}TypeID, xSomethingElseID";
...
$InputInterface = "
	...
	" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Code") . "", $Configuration["InputWidth"], null, true), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"]) . "
	" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Symbol") . "", $Configuration["InputInlineWidth"], null, true), "{$Caption}", null, true, $Configuration["FieldCaptionInlineWidth"]) . "
...
$ImportField = [
	...
	new Database\Field("{$Entity}" . ($Field = "Symbol") . "", "{$Field}"),
