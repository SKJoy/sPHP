...
$EM = new EntityManagement($Table[$Entity = "Currency"]);
	...
	if($Table->Get( // Check for duplicate values for UNIQUE columns
		"
			(
				...
				OR	" . ($Column = "{$Entity}Code") . " = '{$Database->Escape($_POST["{$Column}"])}'
			)
			AND	{$PrimaryKey} <> {$ID}
		"
	, null, null, null, null, null, null))$Result = "Same currency exists!";
...
$EM->ListColumn([
	...
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Symbol") . "", "{$Caption}", null, ALIGN_CENTER),
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Major") . "Name", "{$Caption}", null, ALIGN_CENTER),
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Minor") . "Name", "{$Caption}", null, ALIGN_CENTER),
...
if(isset($_POST["btnInput"])){
	...
	$EM->InputUIHTML([
		...
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Symbol") . "", $EM->InputInlineWidth(), null, true), "{$Caption}", null, true, $EM->FieldCaptionInlineWidth()),
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Major") . "Name", $EM->InputWidth(), null, true), "{$Caption}", true, null, $EM->FieldCaptionWidth()),
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Minor") . "Name", $EM->InputInlineWidth(), null, true), "{$Caption}", null, true, $EM->FieldCaptionInlineWidth()),
...
$EM->SearchSQL([
	...
	SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Symbol") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null,
