...
if(isset($_POST["btnImport"])){
	...
	if(isset($_POST["{$Entity}DataFile"])){
		$Table["{$Entity}"]->Import("{$Environment->TempPath()}{$_POST["{$Entity}DataFile"]}", [
			...
			new Database\Field("" . ($Field = "Project") . "ID", "{$Field}", null, $Table["{$Field}"], "{$Field}Name"),
...
if(isset($_POST["btnInput"])){ // Input form or INSERT/UPDATE record
	...
	if(isset($_POST["btnSubmit"])){ // INSERT/UPDATE record
		...
		if($Form->Verify()){ // Form input okay
			...
			if(!$Form->ErrorMessage()){ // Everything looks fine
				...
				foreach(array_filter(explode(",", str_replace(" ", null, "Category"))) as $OptionEntity)if(isset($Table[$IntermediateEntity = "{$Entity}{$OptionEntity}"])){
	...
	SetVariable($Field = "{$Entity}Type", SetVariable("{$Configuration["SearchInputPrefix"]}{$Field}"));
	SetVariable($Field = "Project", SetVariable("{$Configuration["SearchInputPrefix"]}{$Field}"));
	#endregion CUSTOM: Set default values

	$HTMLElement["Project"] = new HTML\UI\Input("Project", $Configuration["InputFullWidth"], null, true);
	$HTMLElement["Event"] = new HTML\UI\Input("Event", $Configuration["InputFullWidth"], null, true);
	...
	print "
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				$Application->URL($_POST["_Script"]), // Submission URL
				"
					" . HTML\UI\Field($HTMLElement["Project"]->HTML(), "Project", false, false, $Configuration["FieldCaptionWidth"]) . "
					" . HTML\UI\Field($HTMLElement["Event"]->HTML(), "Event", true, false, $Configuration["FieldCaptionWidth"]) . "
		...
		<script>
			sJS.DHTML.Autofill.Make(document.getElementById('{$HTMLElement["Project"]->ID()}'), 3, '{$Application->URL("API/V1/AJAXEntity", "Entity=" . ($OptionEntity = "Project") . "")}', '{$OptionEntity}ID', '{$OptionEntity}ID', '{$OptionEntity}LookupCaption');
			sJS.DHTML.Autofill.Make(document.getElementById('{$HTMLElement["Event"]->ID()}'), 3, '{$Application->URL("API/V1/AJAXEntity", "Entity=" . ($OptionEntity = "Event") . "")}', '{$OptionEntity}ID', '{$OptionEntity}ID', '{$OptionEntity}LookupCaption');
		</script>
	";
...
#region List
#region Search SQL
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "ProjectID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "EventID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
...
print "
	" . HTML\UI\Datagrid(
		...
		$Table["{$Entity}"]->Get(isset($SearchSQL) ? implode(" AND ", $SearchSQL) : null, "" . SetVariable("OrderBy", "P.ProjectName ASC, E.EventName") . " " . SetVariable("Order", "ASC") . "", ((SetVariable("Page", 1) - 1) * ($RecordsPerPage = $Configuration["DatagridRowsPerPage"])) + 1, $RecordsPerPage, null, null, false),
		$Application->URL($_POST["_Script"], "" . implode("&", $SearchSQL) . ""),
		$Table["{$Entity}"]->Count(),
		[ // Columns to display
			new HTML\UI\Datagrid\Column("" . ($Caption = "Project") . "Name", "{$Caption}"),
		...
		"
			" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}" . ($Caption = "Project") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get(), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", false, true) . "
			" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}" . ($Caption = "Event") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get(), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", false, true) . "
