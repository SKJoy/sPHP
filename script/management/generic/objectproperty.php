...
if(isset($_POST["btnImport"])){
	...
	if(isset($_POST["{$Entity}DataFile"])){
		$Table["{$Entity}"]->Import("{$Environment->TempPath()}{$_POST["{$Entity}DataFile"]}", [
			new Database\Field("" . ($Field = "Object") . "ID", "{$Field}", null, $Table["{$Field}"], "{$Field}Name"),
			new Database\Field("Object" . ($Field = "Property") . "Name", "{$Field}"),
			new Database\Field("{$Entity}Is" . ($Field = "Read") . "able", "{$Field}"),
			new Database\Field("{$Entity}Is" . ($Field = "Writ") . "able", "{$Field}e"),
			new Database\Field("{$Entity}" . ($Field = "Default") . "Value", "{$Field}"),
			new Database\Field("{$Entity}Is" . ($Field = "Active") . "", "{$Field}"),
			new Database\Field("{$Entity}ConstructArgument" . ($Field = "Order") . "", "{$Field}", 100),
			//new Database\Field("Is" . ($Field = "Temp") . "", "{$Field}", 1),
		]);
...
if(isset($_POST["btnInput"])){ // Input form or INSERT/UPDATE record
	...
	if(isset($_POST["btnSubmit"])){ // INSERT/UPDATE record
		$Form = new HTML\UI\Form(null, null, null, $Application->EncryptionKey(), null, null, null, null,
			...
			[
				...
				new HTTP\InputValidation("ObjectID", true, VALIDATION_TYPE_INTEGER),
				new HTTP\InputValidation("{$Entity}ConstructArgumentOrder", true, VALIDATION_TYPE_NUMBER),
		...
		if($Form->Verify()){ // Form input okay
			if($Table["{$Entity}"]->Get( // Check for duplicate values for UNIQUE columns
				"
					(
						...
						AND	{$Table["{$Entity}"]->Alias()}.ObjectID = " . intval($_POST["ObjectID"]) . "
					)
			...
			if(!$Form->ErrorMessage()){ // Everything looks fine
				...
				$Table["{$Entity}"]->Put([ // Save information into database
					...
					($Field = "ObjectID")=>$_POST["{$Field}"],
					($Field = "{$Entity}Description")=>$_POST["{$Field}"],
					($Field = "{$Entity}IsReadable")=>$_POST["{$Field}"],
					($Field = "{$Entity}IsWritable")=>$_POST["{$Field}"],
					($Field = "{$Entity}DefaultValue")=>$_POST["{$Field}"],
					($Field = "{$Entity}ConstructArgumentOrder")=>$_POST["{$Field}"],
				...
				foreach(array_filter(explode(",", str_replace(" ", null, "DataType"))) as $OptionEntity){
					$Table[$IntermediateEntity = "{$Entity}{$OptionEntity}"]->Remove("{$Entity}ID = {$AffectedRecord["{$Entity}ID"]}");
					$OptionData = [];
	...
	SetVariable($Field = "ObjectID", SetVariable("{$Configuration["SearchInputPrefix"]}{$Field}"));
	#endregion CUSTOM: Set default values

	// Load option data from intermediate table
	foreach(array_filter(explode(",", str_replace(" ", null, "DataType"))) as $OptionEntity){
		$OptionRecordset = $Table[$IntermediateEntity = "{$Entity}{$OptionEntity}"]->Get("{$Entity}ID = {$EntityID} AND {$Entity}{$OptionEntity}IsActive = 1");
		...
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				...
				"
					...
					" . HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Object") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", true, false, $Configuration["FieldCaptionWidth"]) . "
					" . HTML\UI\Field(HTML\UI\RadioGroup("{$Entity}Is" . ($Caption = "Readable") . "", [new HTML\UI\Radio(1, "Yes"), new HTML\UI\Radio(0, "No")]), "{$Caption}", true, false, $Configuration["FieldCaptionWidth"]) . "
					" . HTML\UI\Field(HTML\UI\RadioGroup("{$Entity}Is" . ($Caption = "Writable") . "", [new HTML\UI\Radio(1, "Yes"), new HTML\UI\Radio(0, "No")]), "{$Caption}", false, true) . "
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}ConstructArgument" . ($Caption = "Order") . "", 80, null, true, INPUT_TYPE_NUMBER), "{$Caption}", false, true) . "
					" . HTML\UI\Field(HTML\UI\CheckboxGroup("" . ($Caption = "Data") . "TypeID", $Table[$OptionEntity = "{$Caption}Type"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "DataTypeLookupCaption", null, "DataTypeID"), "{$Caption} type", true, false, $Configuration["FieldCaptionWidth"]) . "
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Default") . "Value", $Configuration["InputFullWidth"], null, null), "{$Caption}", true, false, $Configuration["FieldCaptionWidth"], null, null, null, "Optional") . "
...
#region List
#region Search SQL
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($SearchColumn = "{$Entity}Name") . "", SetVariable($SearchColumn)) ? "{$Table["{$Entity}"]->Alias()}.{$SearchColumn} LIKE '%{$_POST["{$Configuration["SearchInputPrefix"]}{$SearchColumn}"]}%'" : null;
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($SearchColumn = "{$Entity}Description") . "", SetVariable($SearchColumn)) ? "{$Table["{$Entity}"]->Alias()}.{$SearchColumn} LIKE '%{$_POST["{$Configuration["SearchInputPrefix"]}{$SearchColumn}"]}%'" : null;
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($SearchColumn = "ObjectID") . "", SetVariable($SearchColumn)) ? "{$Table["{$Entity}"]->Alias()}.{$SearchColumn} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$SearchColumn}"]) . "" : null;
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($SearchColumn = "{$Entity}IsReadable") . "", SetVariable($SearchColumn, "")) !== "" ? "{$Table["{$Entity}"]->Alias()}.{$SearchColumn} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$SearchColumn}"]) . "" : null;
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($SearchColumn = "{$Entity}IsWritable") . "", SetVariable($SearchColumn, "")) !== "" ? "{$Table["{$Entity}"]->Alias()}.{$SearchColumn} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$SearchColumn}"]) . "" : null;
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($SearchColumn = "{$Entity}IsActive") . "", SetVariable($SearchColumn, "")) !== "" ? "{$Table["{$Entity}"]->Alias()}.{$SearchColumn} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$SearchColumn}"]) . "" : null;
...
print "
	" . HTML\UI\Datagrid(
		...
		$Table["{$Entity}"]->Get(isset($SearchSQL) ? implode(" AND ", $SearchSQL) : null, "" . SetVariable("OrderBy", "{$Entity}ConstructArgumentOrder") . " " . SetVariable("Order", "ASC") . "", ((SetVariable("Page", 1) - 1) * ($RecordsPerPage = $Configuration["DatagridRowsPerPage"])) + 1, $RecordsPerPage, null, null, false),
		...
		[ // Columns to display
			...
			new HTML\UI\Datagrid\Column("{$Entity}ConstructArgument" . ($Caption = "Order") . "", "{$Caption}"),
			new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Name") . "", "{$Caption}"),
			new HTML\UI\Datagrid\Column("" . ($Caption = "Data") . "TypeName", "{$Caption} type", null, ALIGN_CENTER),
			new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Default") . "Value", "{$Caption}"),
			new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Description") . "", "{$Caption}"),
			new HTML\UI\Datagrid\Column("" . ($Caption = "Object") . "LookupCaption", "{$Caption}"),
			new HTML\UI\Datagrid\Column("{$Entity}Is" . ($Caption = "Read") . "able", "{$Caption}", FIELD_TYPE_BOOLEANICON),
			new HTML\UI\Datagrid\Column("{$Entity}Is" . ($Caption = "Writ") . "able", "{$Caption}e", FIELD_TYPE_BOOLEANICON),
		...
		"
			...
			" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}" . ($Caption = "Object") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("O.{$OptionEntity}ID IN (SELECT DISTINCT(OP.{$OptionEntity}ID) FROM sphp_{$LowercaseEntity} AS OP)"), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", false, true) . "
			" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}{$Entity}Is" . ($Caption = "Read") . "able", [new Option(), new Option(0, "No"), new Option(1, "Yes")]), "{$Caption}", false, true) . "
			" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}{$Entity}Is" . ($Caption = "Writ") . "able", [new Option(), new Option(0, "No"), new Option(1, "Yes")]), "{$Caption}e", false, true) . "
