...
if(isset($_POST["btnImport"])){
	...
	if(isset($_POST["{$Entity}DataFile"])){
		$Table["{$Entity}"]->Import("{$Environment->TempPath()}{$_POST["{$Entity}DataFile"]}", [
			...
			new Database\Field("{$Entity}" . ($Field = "Type") . "ID", "{$Field}", null, $Table["{$Field}"], "{$Field}Name"),
			new Database\Field("" . ($Field = "Bank") . "ID", "{$Field}", null, $Table["{$Field}"], "{$Field}Name"),
			new Database\Field("{$Entity}" . ($Field = "Branch") . "", "{$Field}"),
...
if(isset($_POST["btnInput"])){ // Input form or INSERT/UPDATE record
	...
	if(isset($_POST["btnSubmit"])){ // INSERT/UPDATE record
		$Form = new HTML\UI\Form(null, null, null, $Application->EncryptionKey(), null, null, null, null,
			...
			[
				...
				new HTTP\InputValidation("BankID", true, VALIDATION_TYPE_INTEGER),
		...
		if($Form->Verify()){ // Form input okay
			if($Table["{$Entity}"]->Get( // Check for duplicate values for UNIQUE columns
				"
					(
							{$Entity}Name = '{$Database->Escape($_POST["{$Entity}Name"])}'
						AND	{$Entity}Number = '{$Database->Escape($_POST["{$Entity}Number"])}'
						AND	BA.BankID = " . intval($_POST["BankID"]) . "
					)
			...
			if(!$Form->ErrorMessage()){ // Everything looks fine
				...
				$Table["{$Entity}"]->Put([ // Save information into database
					...
					($Field = "{$Entity}Number")=>$_POST["{$Field}"],
					($Field = "{$Entity}TypeID")=>$_POST["{$Field}"],
					($Field = "BankID")=>$_POST["{$Field}"],
					($Field = "{$Entity}Branch")=>$_POST["{$Field}"],
	...
	SetVariable($Field = "BankID", SetVariable("{$Configuration["SearchInputPrefix"]}{$Field}"));
		...
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				...
				"
					...
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Number") . "", $Configuration["InputInlineWidth"], null, true), "{$Caption}", null, true, $Configuration["FieldCaptionInlineWidth"]) . "
					" . HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Bank") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"]) . "
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Branch") . "", $Configuration["InputWidth"]), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"]) . "
					" . HTML\UI\Field(HTML\UI\Select("{$Entity}" . ($Caption = "Type") . "ID", $Table[$OptionEntity = "{$Entity}{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", true, false, $Configuration["FieldCaptionWidth"]) . "
...
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Number") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]}%'" : null;
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}TypeID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "BankID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Branch") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]}%'" : null;
...
print "
	" . HTML\UI\Datagrid(
		...
		[ // Columns to display
			...
			new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Number") . "", "{$Caption}"),
			new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Type") . "Name", "{$Caption}"),
			new HTML\UI\Datagrid\Column("" . ($Caption = "Bank") . "Name", "{$Caption}"),
			new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Branch") . "", "{$Caption}"),
		...
		"
			...
			" . HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Number") . "", 200), "{$Caption}", false, true) . "
			" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Type") . "ID", $Table[$OptionEntity = "{$Entity}{$Caption}"]->Get(), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", false, true) . "
			" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}" . ($Caption = "Bank") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get(), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", false, true) . "
			" . HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Branch") . "", 200), "{$Caption}", false, true) . "
