			...
			if($Table["{$Entity}"]->Get( // Check for duplicate values for UNIQUE columns
				"
					(
						...
						OR	{$Entity}Identifier = '{$Database->Escape($_POST["{$Entity}Identifier"])}'
					)
		...
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				...
				"
					...
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Identifier") . "", $Configuration["InputFullWidth"], null, true), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"]) . "
...
$SearchSQL[] = strlen(SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Name") . "", SetVariable($Column))) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null;
$SearchSQL[] = strlen(SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Identifier") . "", SetVariable($Column))) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null;
$SearchSQL[] = strlen(SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}IsActive") . "", SetVariable($Column, ""))) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
...
print "
	" . HTML\UI\Datagrid(
		...
		"
			...
			" . HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Identifier") . "", 200), "{$Caption}", false, true) . "
