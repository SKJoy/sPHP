			...
			if($Table["{$Entity}"]->Get( // Check for duplicate values for UNIQUE columns
				"
					(
						...
						AND	B.CountryID = " . intval($_POST["CountryID"]) . "
					)
		...
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				...
				"
					...
					" . HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Country") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", true, false, $Configuration["FieldCaptionWidth"]) . "
...
print "
	...
	" . HTML\UI\Datagrid(
		...
		"{$Entity}ID",
		[ // Action
			new HTML\UI\Datagrid\Action("{$Environment->IconURL()}{$LowercaseEntity}account.png", null, $Application->URL("Management/Generic/{$Entity}Account")),
