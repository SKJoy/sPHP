...
if(isset($_POST["btnInput"])){ // Input form or INSERT/UPDATE record
	...
	$IntermediateEntityList = ListToArray("{$Entity}Form, xEvent");
	...
	if(isset($_POST["btnSubmit"])){ // INSERT/UPDATE record
		...
		if($Form->Verify()){ // Form input okay
			...
			if(!$Form->ErrorMessage()){ // Everything looks fine
				...
				// Create thumbnail
				foreach(array_intersect($Utility->ListToArray("{$Entity}Picture, x{$Entity}Avatar"), $FilePOSTKey) as $Column){
	...
	foreach($IntermediateEntityList as $OptionEntity)if(isset($Table[$IntermediateEntity = "{$Entity}{$OptionEntity}"])){
		$OptionRecordset = $Table[$IntermediateEntity]->Get("{$Table[$IntermediateEntity]->Alias()}.{$Entity}ID = {$EntityID} AND {$Entity}{$OptionEntity}IsActive = 1", null, null, null, null, null, null);
		...
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				...
				"
					...
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Generic") . "Name", $Configuration["InputFullWidth"], null, true), "{$Caption} name", true, null, $Configuration["FieldCaptionWidth"]) . "
...
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}GenericName") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null;
...
print "
	" . HTML\UI\Datagrid(
		...
		[ // Columns to display
			...
			new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Category") . "Name", "{$Caption}"),
			new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Subcategory") . "Name", "{$Caption}"),
		...
		"
			...
			" . HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Generic") . "Name", 200), "{$Caption} name", null, true) . "
