...
$EM = new EntityManagement($Table[$Entity = "Product"]);

$EM->ImportField([
	new Database\Field("{$Entity}" . ($Field = "Name") . "", "{$Field}"),
	new Database\Field("{$Entity}" . ($Field = "Generic") . "Name", "{$Field}"),
	new Database\Field("{$Entity}" . ($Field = "HS") . "Code", "{$Field}"),
	...
	if($Table->Get( // Check for duplicate values for UNIQUE columns
		"
			(
				...
				AND	" . ($Column = "{$Entity}GenericName") . " = '{$Database->Escape($_POST["{$Column}"])}'
				AND	" . ($Column = "{$Entity}HSCode") . " = '{$Database->Escape($_POST["{$Column}"])}'
			)
			AND	{$PrimaryKey} <> {$ID}
		"
	, null, null, null, null, null, null))$Result = "Same product exists!";

	return $Result;
});

$EM->ThumbnailColumn("{$Entity}Picture, x{$Entity}Picture");
...
$EM->IntermediateEntity("{$Entity}Form, x{$Entity}Event");
$EM->DefaultFromSearchColumn("x{$Entity}PlatformID");
...
$EM->ListColumn([
	...
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Generic") . "Name", "{$Caption}"),
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "HS") . "Code", "{$Caption}"),
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Form") . "Name", "{$Caption}"),
...
if(isset($_POST["btnInput"])){
	...
	$EM->InputUIHTML([
		...
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Generic") . "Name", $EM->InputFullWidth(), null, null), "{$Caption}", true, null, $EM->FieldCaptionWidth()),
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "HS") . "Code", $EM->InputWidth(), null, null), "{$Caption} code", true, null, $EM->FieldCaptionWidth()),
		HTML\UI\Field(HTML\UI\CheckboxGroup("{$Entity}" . ($Caption = "Form") . "ID", $Table[$OptionEntity = "{$Entity}{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption", null, "{$OptionEntity}ID"), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"], $EM->FieldContentFullWidth()),
...
$EM->SearchSQL([
	...
	SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Generic") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null,
	SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}HSCode") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null,
	//SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}FormID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null,
...
$EM->SearchUIHTML([
	...
	HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "HS") . "Code", 200), "{$Caption} code", null, null),
	HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Description") . "", 200), "{$Caption}", null, true),
	//HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Form") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get(), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", null, true),
