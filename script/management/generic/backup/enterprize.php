...
if(isset($_POST["btnInput"])){ // Input form or INSERT/UPDATE record
	...
	SetVariable($Field = "CountryID", SetVariable("{$Configuration["SearchInputPrefix"]}{$Field}"));
		...
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				...
				"
					...
					" . HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Country") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"]) . "
...
print "
	...
	" . HTML\UI\Datagrid(
		...
		"
			...
			" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}" . ($Caption = "Country") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get(), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", false, true) . "
