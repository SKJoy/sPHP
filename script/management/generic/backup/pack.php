...
if(isset($_POST["btnImport"])){
	...
	if(isset($_POST["{$Entity}DataFile"])){
		$Table["{$Entity}"]->Import("{$Environment->TempPath()}{$_POST["{$Entity}DataFile"]}", [
			...
			new Database\Field("{$Entity}" . ($Field = "Size") . "", "{$Field}"),
	...
	if(isset($_POST["btnSubmit"])){ // INSERT/UPDATE record
		$Form = new HTML\UI\Form(null, null, null, $Application->EncryptionKey(), null, null, null, null,
			...
			[
				...
				new HTTP\InputValidation("{$Entity}Size", true, VALIDATION_TYPE_INTEGER),
		...
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				...
				"
					...
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Size") . "", 100, null, true, INPUT_TYPE_NUMBER), "{$Caption}", false, true) . "
...
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Size") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
...
print "
	...
	" . HTML\UI\Datagrid(
		...
		[ // Columns to display
			...
			new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Size") . "", "{$Caption}", FIELD_TYPE_NUMBER),
		...
		"
			...
			" . HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Size") . "", 200), "{$Caption}", false, true) . "
