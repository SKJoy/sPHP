...
if(isset($_POST["btnImport"])){
	...
	if(isset($_POST["{$Entity}DataFile"])){
		$Table["{$Entity}"]->Import("{$Environment->TempPath()}{$_POST["{$Entity}DataFile"]}", [
			...
			new Database\Field("" . ($Field = "Namespace") . "ID", "{$Field}", null, $Table["{$Field}"], "{$Field}Name"),
...
if(isset($_POST["btnInput"])){ // Input form or INSERT/UPDATE record
	...
	SetVariable($Field = "Namespace", SetVariable("{$Configuration["SearchInputPrefix"]}{$Field}"));
		...
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				...
				"
					...
					" . HTML\UI\Field(HTML\UI\Select("{$Entity}" . ($Caption = "Type") . "ID", $Table[$OptionEntity = "{$Entity}{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", true, false, $Configuration["FieldCaptionInlineWidth"]) . "
					" . HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Namespace") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", false, true) . "
...
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "NamespaceID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
...
print "
	" . HTML\UI\Datagrid(
		...
		[ // Columns to display
			...
			new HTML\UI\Datagrid\Column("" . ($Caption = "Namespace") . "Name", "{$Caption}"),
		...
		"
			...
			" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}" . ($Caption = "Namespace") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get(), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", false, true) . "
