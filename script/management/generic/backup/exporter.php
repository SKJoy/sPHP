...
$EM = new EntityManagement($Table[$Entity = "Exporter"]);
...
$EM->ValidateInput(function($Entity, $Database, $Table, $PrimaryKey, $ID){
	...
	if($Table->Get( // Check for duplicate values for UNIQUE columns
		"
			...
			AND	{$PrimaryKey} <> {$ID}
		"
	, null, null, null, null, null, null))$Result = "Same exporter in same country for the same department exists!";
...
$EM->ListColumn([
	...
	//new HTML\UI\Datagrid\Column("{$Entity}Address" . ($Caption = "Street") . "", "{$Caption}"),
	//new HTML\UI\Datagrid\Column("{$Entity}Address" . ($Caption = "City") . "", "{$Caption}"),
	//new HTML\UI\Datagrid\Column("{$Entity}Address" . ($Caption = "State") . "", "{$Caption}"),
	//new HTML\UI\Datagrid\Column("{$Entity}Address" . ($Caption = "ZIP") . "", "{$Caption}", null, ALIGN_CENTER),
	new HTML\UI\Datagrid\Column("{$Entity}Address" . ($Caption = "Country") . "Name", "{$Caption}", null, ALIGN_CENTER),
	new HTML\UI\Datagrid\Column("" . ($Caption = "Department") . "LookupCaption", "{$Caption}", null, null),
	new HTML\UI\Datagrid\Column("{$Entity}Is" . ($Caption = "Active") . "", "{$Caption}", FIELD_TYPE_BOOLEANICON),
]);

$EM->Action([
	new HTML\UI\Datagrid\Action("{$Environment->IconURL()}" . strtolower($ActionEntity = "Export") . ".png", null, $Application->URL("Management/Generic/{$ActionEntity}"), null, null, null, "{$ActionEntity}"),
...
if(isset($_POST["btnInput"])){
	...
	$EM->InputUIHTML([
		...
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Logo") . "", $EM->InputFullWidth(), "{$Environment->UploadURL()}{$EM->LowercaseEntityName()}/" . (isset($_POST["{$Entity}{$Caption}"]) ? $_POST["{$Entity}{$Caption}"] : null) . "", null, INPUT_TYPE_FILE), "{$Caption}", true, null, $EM->FieldCaptionWidth()),
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Email") . "", $EM->InputWidth(), null, null, INPUT_TYPE_EMAIL), "{$Caption}", true, null, $EM->FieldCaptionWidth()),
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Phone") . "", $EM->InputInlineWidth(), null, null, INPUT_TYPE_PHONE), "{$Caption}", null, true, $EM->FieldCaptionInlineWidth()),
...
$EM->SearchSQL([
	...
	"E.DepartmentID IN (SELECT SP.DepartmentID FROM sphp_staffpost AS SP WHERE SP.StaffID IN (SELECT S.StaffID FROM sphp_staff AS S WHERE S.UserID = {$User->ID()}))",
