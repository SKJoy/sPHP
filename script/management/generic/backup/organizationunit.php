...
if(isset($_POST["btnInput"])){ // Input form or INSERT/UPDATE record
	...
	if(isset($_POST["btnSubmit"])){ // INSERT/UPDATE record
		$Form = new HTML\UI\Form(null, null, null, $Application->EncryptionKey(), null, null, null, null,
			...
			[
				...
				new HTTP\InputValidation("{$Entity}ShortName", true),
		...
		if($Form->Verify()){ // Form input okay
			...
			if(!$Form->ErrorMessage()){ // Everything looks fine
				...
				$Table["{$Entity}"]->Put([ // Save information into database
					...
					($Field = "OrganizationID")=>$_POST["{$Field}"],
	...
	SetVariable($Field = "OrganizationID", SetVariable("{$Configuration["SearchInputPrefix"]}{$Field}"));
		...
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				...
				"
					...
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Short") . "Name", $Configuration["InputInlineWidth"]), "{$Caption}", null, true, $Configuration["FieldCaptionInlineWidth"]) . "
...
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "OrganizationID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
...
print "
	...
	" . HTML\UI\Datagrid(
		...
		[ // Columns to display
			...
			new HTML\UI\Datagrid\Column("" . ($Caption = "Organization") . "LookupCaption", "{$Caption}"),
