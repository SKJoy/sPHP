...
if(isset($_POST["btnImport"])){
	...
	if(isset($_POST["{$Entity}DataFile"])){
		$Table["{$Entity}"]->Import("{$Environment->TempPath()}{$_POST["{$Entity}DataFile"]}", [
			...
			new Database\Field("{$Entity}" . ($Field = "ShortName") . "", "{$Field}"),
			new Database\Field("" . ($Field = "Enterprize") . "ID", "{$Field}", null, $Table["{$Field}"], "{$Field}Name"),
...
if(isset($_POST["btnInput"])){ // Input form or INSERT/UPDATE record
	...
	if(isset($_POST["btnSubmit"])){ // INSERT/UPDATE record
		$Form = new HTML\UI\Form(null, null, null, $Application->EncryptionKey(), null, null, null, null,
			...
			[
				...
				new HTTP\InputValidation("EnterprizeID", true, VALIDATION_TYPE_INTEGER),
		...
		if($Form->Verify()){ // Form input okay
			if($Table["{$Entity}"]->Get( // Check for duplicate values for UNIQUE columns
				"
					(
						...
						AND	{$Table["{$Entity}"]->Alias()}." . ($Column = "EnterprizeID") . " = " . intval($_POST["{$Column}"]) . "
					)
			...
			if(!$Form->ErrorMessage()){ // Everything looks fine
				...
				$Table["{$Entity}"]->Put([ // Save information into database
					...
					($Field = "EnterprizeID")=>$_POST["{$Field}"],
					($Field = "{$Entity}Logo")=>$_POST["{$Field}"],
	...
	SetVariable($Field = "EnterprizeID", SetVariable("{$Configuration["SearchInputPrefix"]}{$Field}"));
		...
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				...
				"
					...
					" . HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Enterprize") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1"), null, "{$OptionEntity}LookupCaption"), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"]) . "
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Logo") . "", $Configuration["InputFullWidth"], "{$Environment->UploadURL()}{$LowercaseEntity}/" . (isset($_POST["{$Entity}Icon"]) ? $_POST["{$Entity}Icon"] : null) . "", null, INPUT_TYPE_FILE), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"]) . "
...
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "EnterprizeID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
...
print "
	...
	" . HTML\UI\Datagrid(
		...
		$Table["{$Entity}"]->Count(),
		[ // Columns to display
			new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Logo") . "URL", "{$Caption}", FIELD_TYPE_PICTURE),
			...
			new HTML\UI\Datagrid\Column("" . ($Caption = "Enterprize") . "LookupCaption", "{$Caption}"),
		...
		"{$Entity}ID",
		[ // Action
			new HTML\UI\Datagrid\Action("{$Environment->IconURL()}organizationunit.png", null, $Application->URL("Management/Generic/{$Entity}Unit")),
		...
		"
			...
			" . HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Short") . "Name", 200), "{$Caption}", false, true) . "
			" . HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}" . ($Caption = "Enterprize") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get(), new Option(), "{$OptionEntity}LookupCaption"), "{$Caption}", false, true) . "
