...
if(isset($_POST["btnImport"])){
	...
	if(isset($_POST["{$Entity}DataFile"])){
		$Table["{$Entity}"]->Import("{$Environment->TempPath()}{$_POST["{$Entity}DataFile"]}", [
			...
			new Database\Field("{$Entity}Time" . ($Field = "Begin") . "", "{$Field}"),
			new Database\Field("{$Entity}Time" . ($Field = "Stop") . "", "{$Field}"),
	...
	if(isset($_POST["btnSubmit"])){ // INSERT/UPDATE record
		$Form = new HTML\UI\Form(null, null, null, $Application->EncryptionKey(), null, null, null, null,
			...
			[
				...
				new HTTP\InputValidation("{$Entity}TimeStop", true, VALIDATION_TYPE_DATE),
...
//var_dump($_POST); exit;
				$Table["{$Entity}"]->Put([ // Save information into database
					...
					($Field = "{$Entity}Instruction")=>$_POST["{$Field}"],
					($Field = "{$Entity}Picture")=>$_POST["{$Field}"],
					($Field = "{$Entity}Banner")=>$_POST["{$Field}"],
					($Field = "{$Entity}DateBegin")=>$_POST["{$Field}"],
					($Field = "{$Entity}DateEnd")=>$_POST["{$Field}"],
					//($Field = "PlatformID")=>$_POST["{$Field}"],
					($Field = "{$Entity}IsActive")=>$_POST["{$Field}"],
				], $EntityID ? "{$Entity}ID = {$EntityID}" : null, null, true);
		...
		<div class=\"AlignCenter\">
			" . HTML\UI\Form(
				...
				"
					...
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}Date" . ($Caption = "Begin") . "", $Configuration["InputDateWidth"], null, true, INPUT_TYPE_DATE), "{$Caption}", true, false, $Configuration["FieldCaptionWidth"]) . "
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}Date" . ($Caption = "End") . "", $Configuration["InputDateWidth"], null, true, INPUT_TYPE_DATE), "{$Caption}", false, true, null) . "
					...
					" . HTML\UI\Field(HTML\UI\Textarea("{$Entity}" . ($Caption = "Instruction") . "", $Configuration["InputFullWidth"], $Configuration["TextareaHeight"]), "{$Caption}", true, null, $Configuration["FieldCaptionWidth"], null, null, null, "Optional") . "
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Picture") . "", $Configuration["InputFullWidth"], "{$Environment->UploadURL()}{$LowercaseEntity}/" . (isset($_POST["{$Entity}Icon"]) ? $_POST["{$Entity}Icon"] : null) . "", null, INPUT_TYPE_FILE), "{$Caption}", true, false, $Configuration["FieldCaptionWidth"]) . "
					" . HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Banner") . "", $Configuration["InputFullWidth"], "{$Environment->UploadURL()}{$LowercaseEntity}/" . (isset($_POST["{$Entity}Icon"]) ? $_POST["{$Entity}Icon"] : null) . "", null, INPUT_TYPE_FILE), "{$Caption}", true, false, $Configuration["FieldCaptionWidth"]) . "
...
$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Instruction") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]}%'" : null;
//$SearchSQL[] = SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "PlatformID") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} = " . intval($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"]) . "" : null;
...
print "
	" . HTML\UI\Datagrid(
		...
		[ // Columns to display
			...
			new HTML\UI\Datagrid\Column("{$Entity}Date" . ($Caption = "End") . "", "{$Caption}", FIELD_TYPE_DATE),
		...
		"
			...
			" . HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Instruction") . "", 200), "{$Caption}", false, true) . "
