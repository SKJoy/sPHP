				...
				*/
				...
				setup : function(ed) {
					...
					ed.addButton('userFileBrowser', {
						title : 'File browser',
						image : '{$this->Property["Terminal"]->Environment()->URL()}image/icon/filemanager.png',
						onclick : function() {
							// Add you own code to execute something on click
							//OpenUserFileManager();
							alert('" . str_replace("\\", "\\\\", __FILE__) . ":" . __LINE__ . "\\n\\nUpdate TinyMCE loader custom code to launch the file browser.');
						}
					});

					ed.addButton('userExternalResourcePostImageOrg', {title: 'Post Image', image: '{$this->Property["Terminal"]->Environment()->URL()}image/icon/site/postimage.org.png', onclick: function(){window.open('http://www.PostImage.Org');}});
					ed.addButton('userExternalResourceFileDropperCom', {title: 'File Dropper', image: '{$this->Property["Terminal"]->Environment()->URL()}image/icon/site/filedropper.com.png', onclick: function(){window.open('http://www.FileDropper.Com');}});
				},
				...
				content_css : \"{$this->Property["Terminal"]->Environment()->URL()}style/tinymcecontent.css\",
